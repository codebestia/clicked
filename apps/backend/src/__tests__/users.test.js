...
describe('GET /users/:id', () => {
    ...
    it('returns the user profile with wallets on success', async () => {
        vi.mocked(db.query.users.findFirst).mockResolvedValue(MOCK_USER);
    ...
    it('strips internal fields even if db returns them', async () => {
        ...
        vi.mocked(db.query.users.findFirst).mockResolvedValue(userWithInternals);
        const res = await request(app).get('/users/user-uuid-123').set('Authorization', AUTH_HEADER);
        expect(res.status).toBe(200);
...
describe('GET /users/search', () => {
    beforeEach(() => {
        ...
        vi.mocked(db.select).mockReturnValue(chain); // eslint-disable-line
    });
    ...
    it('caps results at 10 via the query limit', async () => {
        vi.mocked(db.query.users.findMany).mockResolvedValue([]); // eslint-disable-line
        await request(app).get('/users/search?q=test').set('Authorization', AUTH_HEADER);
        expect(vi.mocked(db.query.users.findMany)).toHaveBeenCalledWith(expect.objectContaining({ limit: 10 }));
    });
});
