...
usersRouter.get('/search', async (req, res) => {
    ...
    try {
        const results = await db.query.users.findMany({
            where: or(ilike(users.username, prefix), exists(db
                .select({ one: sql `1` })
                .from(wallets)
...
usersRouter.get('/:id/presence', async (req, res) => {
    const id = req.params['id'];
    if (!redis) {
 ...
 */
usersRouter.get('/:id/key-fingerprint', async (req, res) => {
    const id = req.params['id'];
    try {
        ...
        function bytesToSafetySegment(buf, offset, length) {
            let value = BigInt(0);
