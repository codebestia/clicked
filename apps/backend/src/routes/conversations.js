...
function serializeConversationMember(member) {
    return {
        ...
        primaryWalletAddress: member.user.wallets.find((wallet) => wallet.isPrimary)?.address ??
...
conversationsRouter.get('/', async (req, res) => {
    ...
    const memberships = (await db.query.conversationMembers.findMany({
        where: and(eq(conversationMembers.userId, userId), showArchived ? undefined : ne(conversationMembers.isArchived, true)),
        with: {
            conversation: conversationRelations,
        },
    }));
    // Single subquery for message counts — no N+1
    const conversationIds = memberships.map((m) => m.conversationId);
    const countRows = conversationIds.length > 0
        ? await db
...
conversationsRouter.post('/:id/members', async (req, res) => {
    ...
    const existingMembership = await db.query.conversationMembers.findFirst({
        where: and(eq(conversationMembers.conversationId, conversationId), eq(conversationMembers.userId, newUserId)),
    });
...
conversationsRouter.get('/:id/search', async (req, res) => {
    const userId = req.auth.userId;
    const conversationId = req.params['id'];
...
conversationsRouter.post('/:id/transfers', async (req, res) => {
    const userId = req.auth.userId;
    const conversationId = req.params['id'];
    if (!conversationId) {
        res.status(400).json({ error: 'Conversation id is required' });
...
conversationsRouter.get('/:id/transfers', async (req, res) => {
    const userId = req.auth.userId;
    const conversationId = req.params['id'];
    if (!conversationId) {
        res.status(400).json({ error: 'Conversation id is required' });
...
conversationsRouter.delete('/:id/leave', async (req, res) => {
    const userId = req.auth.userId;
    const conversationId = req.params['id'];
    if (!conversationId) {
        res.status(400).json({ error: 'Conversation id is required' });
